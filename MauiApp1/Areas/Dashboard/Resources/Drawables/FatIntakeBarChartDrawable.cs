...
{
    public class FatIntakeBarChartDrawable : IDrawable
    {
        public List<FatIntakeViewItem> FatIntakeViewItems { get; set; }
        ...
        private void GenerateChartYAxes(ICanvas canvas, RectF dirtyRect)
        {
            ...
            for (int i = 0; i <= _yAxisNumberOfLines; i++)
            {
                ...
                {
                    string yValue = $"{this.FatIntakeViewItems[i - 1].TransactionDate.ToString("dd/MM")}";
        ...
        private void GenerateBars(ICanvas canvas, RectF dirtyRect)
        {
            ...
            foreach (FatIntakeViewItem caloriesIntakeData in FatIntakeViewItems)
            {
                Color barFillColor = Color.FromArgb("#0072DB");
                Color barUnfillColor = Color.FromArgb("#EEF0F2");

                double percentageValue = xAxisTotalInterval / _xAxisMaxValue * caloriesIntakeData.FatIntakeCount;
